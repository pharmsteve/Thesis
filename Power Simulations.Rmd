...
sims <- 10                                          # Number of simulations to conduct for each N
...
for (j in 1:length(possible.ns)){
  ...
  for (i in 1:sims){
    ...
    tau <- 0.75                                    # Hypothesize treatment effect
...
pow_1$effect <- as.factor(c(0.75))
pow_1
```

## *Simulation for HR 0.80*
```{r}
# Simulation 
...
for (j in 1:length(possible.ns)){
  ...
  for (i in 1:sims){
    Y0 <- rbinom(n=N, 1, 0.0183*4)                 # control potential outcome rate * mean years of follow-up (5 minus 1 year to conservatively account for depletion of susceptible individuals over time.)
    tau <- 0.80                                    # Hypothesize treatment effect
    Y1 <- Y0*tau                                   # treatment potential outcome
    Z.sim <- rbinom(n=N, size=1, prob=.5)          # Do a random assignment
    Y.sim <- Y1*Z.sim + Y0*(1-Z.sim)               # Reveal outcomes according to assignment
    fit.sim <- lm(Y.sim ~ Z.sim)                   # Do analysis (Simple regression)
...
pow_2$effect <- as.factor(c(0.80))
pow_2
```

## *Simulation for HR 0.85*
```{r}
# Simulation 
possible.ns <- seq(from=80000, to=180000, by=10000) # The sample sizes we'll be considering
powers <- rep(NA, length(possible.ns))              # Empty object to collect simulation estimates
...
for (j in 1:length(possible.ns)){
  ...
  for (i in 1:sims){
    Y0 <- rbinom(n=N, 1, 0.0183*4)                 # control potential outcome rate * mean years of follow-up (5 minus 1 year to conservatively account for depletion of susceptible individuals over time.)
    tau <- 0.85                                    # Hypothesize treatment effect
    Y1 <- Y0*tau                                   # treatment potential outcome
    Z.sim <- rbinom(n=N, size=1, prob=.5)          # Do a random assignment
    Y.sim <- Y1*Z.sim + Y0*(1-Z.sim)               # Reveal outcomes according to assignment
    fit.sim <- lm(Y.sim ~ Z.sim)                   # Do analysis (Simple regression)
...
pow_3$effect <- as.factor(c(0.85))
pow_3
```

## *Simulation for HR 0.90*
```{r}
# Simulation 
possible.ns <- seq(from=80000, to=180000, by=10000)   # The sample sizes we'll be considering
powers <- rep(NA, length(possible.ns))                # Empty object to collect simulation estimates
...
for (j in 1:length(possible.ns)){
  ...
  for (i in 1:sims){
    Y0 <- rbinom(n=N, 1, 0.0183*4)                   # control potential outcome rate * mean years of follow-up (5 minus 1 year to conservatively account for depletion of susceptible individuals over time.)
    tau <- 0.9                                       # Hypothesize treatment effect
    Y1 <- Y0*tau                                     # treatment potential outcome
    Z.sim <- rbinom(n=N, size=1, prob=.5)            # Do a random assignment
    Y.sim <- Y1*Z.sim + Y0*(1-Z.sim)                 # Reveal outcomes according to assignment
    fit.sim <- lm(Y.sim ~ Z.sim)                     # Do analysis (Simple regression)
...
pow_4$effect <- as.factor(c(0.90))
pow_4
```

## *Simulation for HR 0.93*
```{r}
# Simulation 
possible.ns <- seq(from=80000, to=180000, by=10000)     # The sample sizes we'll be considering
powers <- rep(NA, length(possible.ns))                  # Empty object to collect simulation estimates
...
for (j in 1:length(possible.ns)){
  ...
  for (i in 1:sims){
    Y0 <- rbinom(n=N, 1, 0.0183*4)                 # control potential outcome rate * mean years of follow-up (5 minus 1 year to conservatively account for depletion of susceptible individuals over time.)
    tau <- 0.93                                    # Hypothesize treatment effect
    Y1 <- Y0*tau                                   # treatment potential outcome
    Z.sim <- rbinom(n=N, size=1, prob=.5)          # Do a random assignment
    Y.sim <- Y1*Z.sim + Y0*(1-Z.sim)               # Reveal outcomes according to assignment
    fit.sim <- lm(Y.sim ~ Z.sim)                   # Do analysis (Simple regression)
...
pow_5$effect <- as.factor(c(0.93))
pow_5
```

## *Simulation for HR 0.95*
```{r}
# Simulation 
possible.ns <- seq(from=80000, to=180000, by=10000)     # The sample sizes we'll be considering
powers <- rep(NA, length(possible.ns))                  # Empty object to collect simulation estimates
...
for (j in 1:length(possible.ns)){
  ...
  for (i in 1:sims){
    ...
    tau <- 0.95                                    # Hypothesize treatment effect
    Y1 <- Y0*tau                                   # treatment potential outcome
    Z.sim <- rbinom(n=N, size=1, prob=.5)          # Do a random assignment
    Y.sim <- Y1*Z.sim + Y0*(1-Z.sim)               # Reveal outcomes according to assignment
    fit.sim <- lm(Y.sim ~ Z.sim)                   # Do analysis (Simple regression)
...
# Plot power curves
ggplot() + 
  geom_line(data = pow_1, aes(x = possible.ns, y = powers, color = "HR=0.75"), linewidth=1)+
  geom_smooth(data = pow_2n, aes(x = possible.ns, y = power_n, color = "HR=0.80"), 
              se = FALSE, method = "gam", formula = y ~ s(log(x), k = 4)) +
  geom_smooth(data = pow_3n, aes(x = possible.ns, y = power_n, color = "HR=0.85"), 
              se = FALSE, method = "gam", formula = y ~ s(log(x), k = 4)) +
  geom_smooth(data = pow_4n, aes(x = possible.ns, y = power_n, color = "HR=0.90"), 
              se = FALSE, method = "gam", formula = y ~ s(log(x), k = 4)) +
  geom_smooth(data = pow_5, aes(x = possible.ns, y = powers, color = "HR=0.93"), 
              se = FALSE, method = "gam", formula = y ~ s(log(x), k = 4)) +
  geom_smooth(data = pow_6, aes(x = possible.ns, y = powers, color = "HR=0.95"), 
              se = FALSE, method = "gam", formula = y ~ s(log(x), k = 4)) +
  labs(x = "Study Sample Size", y = "Power", title = "Power for Objective 1 (5-year follow-up)") + 
  theme_classic() +  
  ...
  scale_color_manual(name = "Effect Size", 
                     values = c("HR=0.75" = "blue", "HR=0.80" = "purple", "HR=0.85" = "green", "HR=0.90" = "brown", "HR=0.93" = "darkgreen" , "HR=0.95" = "darkgrey"),
                     labels = c("HR=0.75", "HR=0.80", "HR=0.85", "HR=0.90", "HR=0.93", "HR=0.95"))
...
for (j in 1:length(possible.ns)){
  ...
  for (i in 1:sims){
    Y0 <- rbinom(n=N, 1, 0.06*4)                   # control potential outcome rate * mean years of follow-up (5 minus 1 year to conservatively account for depletion of susceptible individuals over time.)
    tau <- 0.90                                    # Hypothesize treatment effect
    Y1 <- Y0*tau                                   # treatment potential outcome
    Z.sim <- rbinom(n=N, size=1, prob=.5)          # Do a random assignment
    Y.sim <- Y1*Z.sim + Y0*(1-Z.sim)               # Reveal outcomes according to assignment
    fit.sim <- lm(Y.sim ~ Z.sim)                   # Do analysis (Simple regression)
...
# Plot power curves
ggplot() + 
  geom_line(data = MACE1, aes(x = possible.ns, y = powers, color = "HR=0.75"), linewidth = 1) +
  geom_smooth(data = MACE2n, aes(x = possible.ns, y = power_n, color = "HR=0.80"), 
              se = FALSE, method = "gam", formula = y ~ s(log(x), k = 4)) +
  geom_smooth(data = MACE3n, aes(x = possible.ns, y = power_n, color = "HR=0.85"), 
              se = FALSE, method = "gam", formula = y ~ s(log(x), k = 4)) +
  geom_smooth(data = MACE4n, aes(x = possible.ns, y = power_n, color = "HR=0.90"), 
              se = FALSE, method = "gam", formula = y ~ s(log(x), k = 4)) +
  geom_smooth(data = MACE5, aes(x = possible.ns, y = powers, color = "HR=0.93"), 
              se = FALSE, method = "gam", formula = y ~ s(log(x), k = 4)) +
  geom_smooth(data = MACE6, aes(x = possible.ns, y = powers, color = "HR=0.95"), 
              se = FALSE, method = "gam", formula = y ~ s(log(x), k = 4)) +
  labs(x = "Study Sample Size", y = "Power", title = "Power for Objective 2 (5-year follow-up)") + 
  theme_classic() +  
  theme(plot.title = element_text(hjust = 0.5),  
        panel.grid = element_blank()) +
  scale_x_continuous(labels = scales::comma) +
